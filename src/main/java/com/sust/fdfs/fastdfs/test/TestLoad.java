...
package com.sust.fdfs.fastdfs.test;

import com.sust.fdfs.fastdfs.*;
	...
	public static class Uploader
	{
  	...
  	public int uploadFile() throws Exception
  	{
  		...
  		try
  		{
	  		...
	  		com.sust.fdfs.fastdfs.test.TestLoad.file_ids.offer(file_id);
	  		return 0;
	  	}
	  	catch(Exception ex)
	  	{
...
*/
	  ...
	  public void run()
    {
    	try
    	{
    		com.sust.fdfs.fastdfs.test.TestLoad.upload_thread_count++;
    		...
    		System.out.println("upload thread " + this.thread_index + " start");
				...
				{
						com.sust.fdfs.fastdfs.test.TestLoad.total_upload_count++;
    				if (uploader.uploadFile() == 0)
    				{
    					com.sust.fdfs.fastdfs.test.TestLoad.success_upload_count++;
    				}
    		}
    	}
	  	catch(Exception ex)
	  	...
	  	System.out.println("upload thread " + this.thread_index
	  	                 + " exit, total_upload_count: " + com.sust.fdfs.fastdfs.test.TestLoad.total_upload_count
	  	                 + ", success_upload_count: " + com.sust.fdfs.fastdfs.test.TestLoad.success_upload_count
	  	                 + ", total_download_count: " + com.sust.fdfs.fastdfs.test.TestLoad.total_download_count
	  	                 + ", success_download_count: " + com.sust.fdfs.fastdfs.test.TestLoad.success_download_count);
    }
  }
...
*/
	  ...
	  public void run()
    {
    	try
    	{
    		...
    		file_id = "";
				while ( com.sust.fdfs.fastdfs.test.TestLoad.upload_thread_count != 0 || file_id != null)
				{
					file_id = (String) com.sust.fdfs.fastdfs.test.TestLoad.file_ids.poll();
					if (file_id == null)
					{
						Thread.sleep(10);
						continue;
					...
					{
						com.sust.fdfs.fastdfs.test.TestLoad.total_download_count++;
					}
    			if (downloader.downloadFile(file_id) == 0)
    			{
    				synchronized (this.counter_lock)
    				{
    					com.sust.fdfs.fastdfs.test.TestLoad.success_download_count++;
    				}
    			}
    			else
    			{
    				com.sust.fdfs.fastdfs.test.TestLoad.fail_download_count++;
    			}
    		}
    		
    		for ( int i = 0; i<3 && com.sust.fdfs.fastdfs.test.TestLoad.total_download_count < com.sust.fdfs.fastdfs.test.TestLoad.total_upload_count; i++)
    		{
					file_id = (String) com.sust.fdfs.fastdfs.test.TestLoad.file_ids.poll();
					if (file_id == null)
					{
	  	...
	  	System.out.println("download thread " + this.thread_index
	  	                 + " exit, total_download_count: " + com.sust.fdfs.fastdfs.test.TestLoad.total_download_count
	  	                 + ", success_download_count: " + com.sust.fdfs.fastdfs.test.TestLoad.success_download_count
	  	                 + ", fail_download_count: " + com.sust.fdfs.fastdfs.test.TestLoad.fail_download_count);
    }
  }
